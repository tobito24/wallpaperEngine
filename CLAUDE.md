...
| `TileGridBase/` | Active — base/template, not a wallpaper itself | A reusable base for any 2D tile-grid project: camera (pan + zoom, tile size in px) over a tile grid, TypeScript + Vite. Meant to be **cloned** as the starting point for new projects rather than developed into a wallpaper directly. See its own `CLAUDE.md` for its forks (in progress: `ColorWar`; not started: a noise/biome terrain-generation wallpaper, this project's original scope under the name `WorldForge` before its 2026-09-13 reset). |
| `ColorWar/` | Active — new (started 2026-09-13) | Cloned from `TileGridBase`. A red/green/blue cyclic-dominance tile automaton (Rock-Paper-Scissors-style, not actually Conway's Game of Life despite the working name it started from). Core simulation rule not yet decided/implemented — see its own `CLAUDE.md`'s "Open design questions". |
...
The lineage so far: `WorldGenerator` (done) → `InfinityWalker` (paused) → `TileGridBase` (active, base/template) → `ColorWar` (active, its first fork). When planning new terrain-generation work, that most likely means cloning `TileGridBase` into a new project, not building directly inside `TileGridBase` itself — see its `CLAUDE.md` for the remaining planned fork.
